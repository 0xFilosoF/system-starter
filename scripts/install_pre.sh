...
if pkg_installed grub && [ -f /boot/grub/grub.cfg ]; then
    ...
    if [ ! -f /etc/default/grub.starter.bkp ] && [ ! -f /boot/grub/grub.starter.bkp ]; then
        sudo cp /etc/default/grub /etc/default/grub.starter.bkp
        sudo cp /boot/grub/grub.cfg /boot/grub/grub.starter.bkp
        ...
        *) grubtheme="None" ;;
        ...
        if [ "${grubtheme}" == "None" ]; then
            ...
            sudo tar -xzf "${cloneDir}/resources/arcs/Grub_${grubtheme}.tar.gz" -C /usr/share/grub/themes/
...
if pkg_installed systemd && nvidia_detect && [ "$(bootctl status 2>/dev/null | awk '{if ($1 == "Product:") print $2}')" == "systemd-boot" ]; then
    ...
    if [ "$(find /boot/loader/entries/ -type f -name '*.conf.starter.bkp' 2>/dev/null | wc -l)" -ne "$(find /boot/loader/entries/ -type f -name '*.conf' 2>/dev/null | wc -l)" ]; then
        ...
        find /boot/loader/entries/ -type f -name "*.conf" | while read -r imgconf; do
            sudo cp "${imgconf}" "${imgconf}.starter.bkp"
...
if [ -f /etc/pacman.conf ] && [ ! -f /etc/pacman.conf.starter.bkp ]; then
    print_log -g "[PACMAN] " -b "modify :: " "adding extra spice to pacman..."

    # shellcheck disable=SC2154
    sudo cp /etc/pacman.conf /etc/pacman.conf.starter.bkp
...
if grep -q '\[chaotic-aur\]' /etc/pacman.conf; then
    ...
    *)
    ...
    if [ "${is_chaotic_aur}" == true ]; then
        ...
        sudo "${STARTER_CLONE_DIR}/utils/chaotic_aur.sh" --install
    else
