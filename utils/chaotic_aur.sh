...
#! REQUIRED ROOT
EXEC_NAME="$0 $*"
ROOT_OPTS=("--install" "--purge" "--revert" "fresh") #? List Of Flags that needs to be in sudo
VERT_L="$(printf '=%.0s' $(seq 1 "$(tput cols)"))"
...
check_Root() {
    if [ "$EUID" -ne 0 ]; then
        echo "Error: [ $EXEC_NAME ] must be run as root!"
        exit 1
    fi
}
